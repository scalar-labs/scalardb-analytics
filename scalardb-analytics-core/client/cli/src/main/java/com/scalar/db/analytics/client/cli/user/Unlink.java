...
@Command(
    name = "unlink",
    description = "Remove the link between an Analytics user and a backend user")
public class Unlink implements Callable<Integer> {
  ...
  @Option(
      names = {"--backend-user"},
      description = "Backend username to unlink",
      required = true)
  @SuppressWarnings("NotNullFieldNotInitialized")
  ...
  public Integer call() {
    try {
      ...
      modules.client().user().unlinkBackendUser(username, backendUsername, backend);
      printer()
          .printSuccess(
              new Success(
                  String.format(
                      "Unlinked user '%s' from backend user '%s' (backend=%s)",
      ...
      printer()
          .printError(
              new CommandError(
                  String.format(
                      "Failed to unlink user '%s' from backend user '%s'",
