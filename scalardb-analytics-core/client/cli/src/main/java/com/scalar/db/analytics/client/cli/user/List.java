...
public class List implements Callable<Integer> {
  ...
  public Integer call() {
    try {
      ...
      java.util.List<UserInfo> users = modules.client().user().listUsers();
      printer().printSuccess(new Success(users));
      return ExitCode.OK;
    } catch (Exception e) {
      printer().printError(new CommandError("Failed to list users", e));
