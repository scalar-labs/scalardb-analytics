...
@Command(name = "revoke", description = "Remove a role assignment from a user")
public class Revoke implements Callable<Integer> {
  ...
  public Integer call() {
    try {
      ...
      modules.client().role().revokeRole(roleName, username);
      printer()
          .printSuccess(
              new Success(String.format("Role %s revoked from user %s", roleName, username)));
      return ExitCode.OK;
    } catch (Exception e) {
      printer()
          .printError(
              new CommandError(
                  String.format("Failed to revoke role %s from user %s", roleName, username), e));
