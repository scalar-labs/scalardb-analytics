...
@Command(name = "namespace", description = "Grant a permission on a namespace")
public class GrantNamespace implements Callable<Integer> {
  @Option(
      names = {"-p", "--permission"},
  ...
  public Integer call() {
    try {
      ...
      printer().printError(new CommandError("Failed to grant permission on namespace", e));
