...
@Command(name = "list", description = "List catalogs")
public class List implements Callable<Integer> {
  ...
  public Integer call() {
    try {
      ...
      java.util.List<Catalog> catalogs = modules.client().catalog().listAllCatalogs();
      printer().printSuccess(new Success(catalogs));
      return ExitCode.OK;
    } catch (Exception e) {
      printer().printError(new CommandError("Failed to list catalogs", e));
