...
@Command(name = "table", description = "Grant a permission on a table")
public class GrantTable implements Callable<Integer> {
  @Option(
      names = {"-p", "--permission"},
  ...
  public Integer call() {
    try {
      ...
      printer().printError(new CommandError("Failed to grant permission on table", e));
