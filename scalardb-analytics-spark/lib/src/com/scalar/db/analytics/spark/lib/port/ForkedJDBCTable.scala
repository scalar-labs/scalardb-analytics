...
// format: off
/** This class is ported from Apache Spark. The original source code can be
  * found at:
  * sql/core/src/main/scala/org/apache/spark/sql/execution/datasources/v2/jdbc/JDBCTable.scala
  * <p> The original license is as follows: <p> Licensed to the Apache Software
  ...
  */
package com.scalar.db.analytics.spark.lib.port
...
/** This class is forked from the Apache Spark codebase
  * [[org.apache.spark.sql.execution.datasources.v2.jdbc.JDBCTable]] as of
  * version 3.5.3. This is required to keep the original class in environments
  * where the providers modify the class implementation, which may lead to
  * runtime errors. So far, Databricks has modified the class.
  */
case class ForkedJDBCTable(
    ident: Identifier,
    schema: StructType,
  ...
  override def newWriteBuilder(info: LogicalWriteInfo): WriteBuilder = {
    val mergedOptions = new JdbcOptionsInWrite(
      jdbcOptions.parameters.originalMap ++ info.options
        .asCaseSensitiveMap()
        .asScala
    )
  ...
  override def createIndex(
    ...
    JdbcUtils.withConnection(jdbcOptions) { conn =>
      JdbcUtils.classifyException(
        s"Failed to create index $indexName in $name",
  ...
  override def dropIndex(indexName: String): Unit = {
    JdbcUtils.withConnection(jdbcOptions) { conn =>
      JdbcUtils.classifyException(
        s"Failed to drop index $indexName in $name",
        JdbcDialects.get(jdbcOptions.url)
      ) {
